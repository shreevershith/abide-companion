...
43. ✅ **Architecture review + three robustness fixes** — full system design review across all pipelines; three highest-impact gaps shipped: (a) **Groq Whisper retry** — `transcribe()` in `app/audio.py` now wraps the `asyncio.wait_for` in a two-attempt loop (1 s gap); a transient Groq stall silently retries instead of dropping the user's utterance. Same pattern as Claude first-token (D97) and TTS first-byte retries. (b) **YAMNet confidence threshold 0.30 → 0.40** in `app/audio_events.py` — 0.30 was generating spurious Cough/Throat-clearing tags from ambient noise and breathing, triggering unnecessary welfare reactions; 0.40 still catches loud real events (score 0.5+ on genuine coughs/sneezes/gasps) while suppressing low-confidence noise. (c) **Deferred `status: listening` on frontend** — server sends `status: listening` immediately after `response_done` while the PCM ring buffer still holds 1-3 s of audio; hero amber glow was disappearing while Abide was still audibly speaking. `pendingListeningStatus` flag defers the `setStatus` call until the `drained` worklet message fires. Remaining gaps documented in D104 (adaptive frame rate, file locking, output safety filter, single-model fallback). (Phase W, D104)
44. ✅ **Security, performance, and error-handling audit — full fix batch** — ten changes across four files from a comprehensive codebase audit: (a) **Server-side Anthropic key for `/api/analyze`** (`main.py`) — `_last_session_anthropic_key` set at WS config time; endpoint no longer depends on client-supplied API key in request body, preventing credential relay. (b) **`/api/analyze` input validation** — history capped at 60 turns × 5 KB/message with role validation; `activity_log` capped at 50 KB; type guards on both. (c) **Split connect/read timeout** — `_analyze_client` now uses `httpx.Timeout(connect=5.0, read=25.0)` instead of combined 30 s. (d) **`face_bbox` geometry validation** — added `x1 < x2` and `y1 < y2` checks; inverted bbox previously passed validation and reached PTZ math with a backwards centroid. (e) **`face_bbox` 15 Hz rate gate** — messages above 15 Hz dropped before validation, protecting event loop from runaway clients. (f) **Whisper retry catches `httpx.TransportError`** (`audio.py`) — connection resets were previously unretried; now catches both `asyncio.TimeoutError` and `httpx.TransportError` with retry log naming the exception type. (g) **Defensive `try/except` in `_log_bg_exception`** (`session.py`) — `task.exception()` can itself raise `CancelledError`; asyncio silently discards callback exceptions. (h) **`CancelledError` reason in log** — distinguishes `"user-interrupted"` from `"network-drop"` based on `self._cancelled`. (i) **Telemetry log level differentiation** — `AttributeError`/`TypeError` in `end_turn_trace` → WARNING; transient Langfuse errors → DEBUG. (j) **`VisionBuffer` 15-min TTL** (`vision.py`) — `as_context()` filters out entries older than 900 s so stale observations from a long break don't pollute Claude's activity context. (Phase X, D105)
45. ✅ **Detection sensitivity bundle** — four fixes from live session `abide-2d0bfd1f838b` showing waving (10-15 s latency), yawning (never detected), and coughing (many attempts needed): (a) **Vision timeout 8 s → 5 s** — each timeout blocks subsequent vision cycles for its full duration; dropping to 5 s cuts blocked cycles from 3 to ≤2 per stall, still 3× over GPT-4.1-mini median. (b) **Yawning as `noteworthy=True` in vision prompt** — added labelled example; the model now flags visible yawns as worth a proactive welfare reaction. (c) **Cough-on-silence pipeline fix** — the empty-transcript branch was explicitly cancelling the YAMNet task, silently dropping every cough/gasp/sneeze that triggered VAD without words. Cancellation removed; done-callback now stores events for next turn. Bonus: if YAMNet has already completed when Whisper returns empty (common — ~280 ms vs ~600 ms+), welfare events (Cough/Sneeze/Gasp/Wheeze) fire an immediate proactive `start_response("")` with 15 s cooldown. (d) **YAMNet threshold 0.40 → 0.35** — genuine coughs were landing in 0.32-0.39 range and being filtered; 0.35 keeps suppression above breath-noise floor (0.15-0.25) while recovering missed welfare signals. (D106)
46. ✅ **TTS mid-stream chunk timeout** — `stream_sentence` previously disabled ALL timeout via `timeout_cm.reschedule(None)` once the first byte arrived, leaving no guard against a throttled OpenAI stream. Live session `abide-350964a5617c` saw a sentence delivered at 7.4 KB/s (vs normal 100+ KB/s), taking 11+ s and causing audible distortion on the next sentence. Fix: replaced `async for chunk in resp.aiter_bytes()` with a manual `while True: await asyncio.wait_for(__anext__, 3.0)` loop. Before first byte: first-byte deadline (`timeout_cm`, 10 s) still covers the wait. After first byte: inter-chunk deadline (`_TTS_CHUNK_TIMEOUT_S = 3.0`) aborts a throttled stream after 3 s of silence, logging `[STALL] TTS mid-stream`. The generator returns cleanly; the `async with client.stream()` context manager closes the response. (D107)
48. ✅ **Security, performance, and error-handling audit — second full-pass batch** — 126 findings triaged across all pipeline files; 9 highest-impact changes: (a) `_log_prewarm_exception` defensive try/except wrapper (`main.py`, matches `session.py:_log_bg_exception`); (b) final memory-flush log level `debug→warning` (`main.py`); (c) Groq 429 rate-limit branch with user-safe message (`main.py`); (d) per-message history cap at 10 KB before `_history.append` (`conversation.py`); (e) Anthropic 429 → specific `ConversationError` (`conversation.py`); (f) vision JSON-parse fallback removed — returns empty SceneResult instead of injecting raw API error text into camera-observations (`vision.py`); (g) memory save failure `debug→warning` (`memory.py`); (h) TTS phrase-store save failure `debug→warning` (`tts_cache_store.py`); (i) OpenAI TTS 429 → skip sentence cleanly instead of raising (`tts.py`). (D109)
47. ✅ **TTS slow-dribble rate-based abort** — the per-chunk 3 s timeout (D107) doesn't catch a throttle pattern where chunks arrive every 1-2 s at 10-21 KB/s (below the 3 s gap threshold but 5-10× below normal 100-200 KB/s). Live session `abide-ee7e5abb250e`: "Ha, just waving hello then..." delivered 125,400 B in 6,000 ms (≈21 KB/s), causing ring-buffer underrun and audible distortion. Fix: after a 1.5 s grace period, compute running rate `total_bytes / elapsed_since_first_byte` on every chunk; if rate < `_TTS_MIN_RATE_BPS` (30 KB/s), abort the generator with `[STALL] TTS slow-dribble`. Short sentences (< ~45 KB) finish before the grace period and are unaffected. (D108)
